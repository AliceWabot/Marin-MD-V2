let handler = async (m, { conn, isOwner, text, isAdmin }) => {
  ...
  try {
    ...
    m.reply(`done! ${await conn.user.name} not active in chat ${await conn.getName(who) == undefined ? 'this' : await conn.getName(who)}.`)
  } catch (e) {
    throw `⚠️ number does not exist in database! ⚠️`
  }
}
handler.help = ['ban']
